...
export const getAllOtherProperties = async (req, res) => {
  try {
    // Check if user is authenticated, if not return all properties
    const userId = req.user ? req.user.id : null;

    let filterQuery = {};
    
    // If user is authenticated, exclude their own properties
    if (userId) {
      filterQuery = {
        userId: { $ne: userId },
      ...
      // If not authenticated, show all properties except rent/lease
      filterQuery = {
        purpose: { $ne: "Rent/Lease" },
      };
    }
